...
export function Dashboard() {
  ...
  // gentle poll so med/consumable numbers don't go stale on a long-lived tab
  useEffect(() => {
    const t = setInterval(() => {
      due.reload();
      consumables.reload();
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="p-4">
          <div className="mb-3 flex items-center justify-between">
            <h2 className="font-medium text-slate-200">Care inbox</h2>
            <Link to="/inbox" className="text-xs text-indigo-400 hover:text-indigo-300">Open →</Link>
          </div>
          {inbox.loading && !inbox.data ? (
            ...
            <p className="py-6 text-center text-sm text-slate-500">Nothing needs attention. 🎉</p>
          )}
        </Card>

        <Card className="p-4">
          <div className="mb-3 flex items-center justify-between">
            <h2 className="font-medium text-slate-200">Supplies</h2>
            <Link to="/ops" className="text-xs text-indigo-400 hover:text-indigo-300">Ops →</Link>
          </div>
          {lowConsumables.length ? (
            <ul className="space-y-2">
              {lowConsumables.map((c) => (
                <li key={c.id} className="flex items-center gap-2 text-sm">
                  <Badge className="bg-amber-500/15 text-amber-300 ring-amber-500/30">{c.status.level}</Badge>
            ...
            <p className="py-6 text-center text-sm text-slate-500">All supplies above threshold.</p>
          )}
        </Card>
      </div>
    </div>
